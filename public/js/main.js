$(function() {
    ...
    const gameVariables = {
        ...
        $playAgain: $(
            '<div class="btn btn-outline-danger white">Next Pokemon</div>',
        ),
        $pokeballs: '<img src="css/images/pokeball.png" class="pokeball-div">',
        $emptyballs:
            '<img src="css/images/pokeball.png" class="pokeball-div hidden">',
        win: 0,
        loss: 0,
    };
    //Start Game
    ...
    function createPuzzle() {
        ...
        gameVariables.lettersInPlay = gameVariables.randomPokemon
            .toUpperCase()
            .split('');
        //display pokemon image AND fade
        var img = $('<img class="hidden img-size img-fluid" />')
            .attr('src', pokemon[gameVariables.randomNumber].img)
            .hide()
            .fadeIn(500);
        ...
        $('#pokebox').append(
            '<button type="button" id ="poke-button" class="btn btn-outline-primary white" data-toggle="modal" data-target="#pokeboxModal">Pokebox</button>',
        );
    ...
    function createKeyboard() {
        ...
        for (var i = 0; i < letters1.length; i++) {
            $keyboard1.append(
                '<div class="btn btn-outline-primary keys white" id="letter-div">' +
                    letters1[i] +
                    '</div>',
            );
        }
    ...
    function playGame(x) {
        ...
        for (var i = 0; i < gameVariables.lettersInPlay.length; i++) {
            if (x === gameVariables.lettersInPlay[i]) {
                ...
                if (
                    gameVariables.$wordDiv.text().replace(/ +/g, '') ===
                    gameVariables.randomPokemon.toUpperCase()
                ) {
                    //show winner user won
                    function winQuote() {
                        alert(
                            'You caught ' + gameVariables.randomPokemon + '!',
                        );
                    }
                    ...
                    var pokeGif = $('<img class="gif-size">').attr(
                        'src',
                        pokemon[gameVariables.randomNumber].gif,
                    );
        ...
        if (gameVariables.lettersInPlay.indexOf(x) === -1) {
            ...
            if (gameVariables.numberOfAttempts === 0) {
                ...
                gameVariables.$wordDiv.html(
                    gameVariables.lettersInPlay.join(' '),
                );
